...
> {
	...
	update = (
		...
		// eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
		const shouldUpdateTransform = !!options.transform?.deps?.some(
		...
		if (!shouldUpdateValues && !shouldUpdateState && !shouldUpdateTransform) return
	...
	validateSync = (
		...
		type CurrentFormErrorMap = FormErrorMapFromValidator<
			TFormData,
			TOnMount,
			TOnChange,
			TOnChangeAsync,
			...
			TOnSubmitAsync
		>
		type ValidatorFunctionType = FormValidateOrFn<TFormData>
		type CurrentValidatorItem = ValidatorItem<
			ValidatorFunctionType,
			ValidationErrorMapKeys,
			unknown
		>
		type CurrentRawError = ReturnType<typeof this.runValidator>
		type CurrentProcessedError = FormValidationError<TFormData> | undefined

		const processErrorsCallback = (
			rawError: CurrentRawError,
			validatorItem: CurrentValidatorItem,
			currentFieldsErrorMap: CurrentFormErrorMap = {},
		): {
			processedError: CurrentProcessedError
			aggregatedErrors: CurrentFormErrorMap
			hasIndividualError: boolean
		} => {
			const { formError, fieldErrors } = normalizeError<TFormData>(rawError)

			const errorMapKey = validatorItem.key
			let hasThisValidatorErrored = false
			const updatedFieldsErrorMap: CurrentFormErrorMap = {
				...currentFieldsErrorMap,
			}
			...
			for (const field of Object.keys(
				...
				if (newSource === 'form' && newFormValidatorError !== undefined) {
					updatedFieldsErrorMap[field] = {
						...(updatedFieldsErrorMap[field] || {}),
						[errorMapKey]: newFormValidatorError,
					} as CurrentFormErrorMap[DeepKeys<TFormData>]
				}
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
				if (currentErrorMap?.[errorMapKey] !== newErrorValue) {
					this.setFieldMeta(field, (prev) => ({
						...prev,
						errorMap: { ...prev.errorMap, [errorMapKey]: newErrorValue },
						errorSourceMap: {
							...prev.errorSourceMap,
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
			if (this.state.errorMap?.[errorMapKey] !== formError) {
				this.baseStore.setState((prev) => ({
					...prev,
					errorMap: { ...prev.errorMap, [errorMapKey]: formError },
				}))
			}

			if (formError || (fieldErrors && Object.keys(fieldErrors).length > 0)) {
				hasThisValidatorErrored = true
			}

			return {
				processedError: formError,
				aggregatedErrors: updatedFieldsErrorMap,
				hasIndividualError: hasThisValidatorErrored,
			}
		}

		const validationRunnerInstance = new ValidationRunner<
			{ value: TFormData; formApi: this }, 
			CurrentValidatorItem, 
			CurrentRawError, 
			CurrentProcessedError,
			ValidationErrorMapKeys,
			CurrentFormErrorMap,
			FormOptions<
				TFormData,
			...
			>
		>(
			(validatorItem, valueContainer) =>
				this.runValidator({
					validate: validatorItem.validate as ValidatorFunctionType,
					value: valueContainer.value, 
					type: 'validate',
					formApi: valueContainer.formApi,
				}),
			processErrorsCallback,
			(valCause, opts) =>
				getSyncValidatorArray(valCause, opts) as CurrentValidatorItem[],
		)

		const { hasErrored: overallHasErrored, errors: finalFieldsErrorMap } =
			validationRunnerInstance.validate(
				cause,
				this.options as FormOptions<
					...
					TSubmitMeta
				>,
				{ value: this.state.values, formApi: this },
			)
		...
		return {
			...
			fieldsErrorMap: finalFieldsErrorMap || {},
		}
	}
